...
impl<'a> VM<'a> {
    ...
    fn execute(&mut self, instruction: Instruction) -> Result<(), String> {
        ...
        match opcode {
            ...
            OP_CONST_I64 => self.const_i64(instruction),
            OP_CONST_F64 => self.const_f64(instruction),
            ...
            OP_ADD_F64 => self.add_f64(instruction),
            OP_SUB_F64 => self.sub_f64(instruction),
            OP_MUL_F64 => self.mul_f64(instruction),
            OP_DIV_F64 => self.div_f64(instruction),
            OP_MOD_F64 => self.mod_f64(instruction),
            OP_POW_F64 => self.pow_f64(instruction),
            OP_NEG_F64 => self.neg_f64(instruction),
            ...
            // casting ops
            OP_I64_TO_F64 => self.i64_to_f64(instruction),
            OP_F64_TO_I64 => self.f64_to_i64(instruction),
            OP_BOX => self.box_value(instruction),
            OP_UNBOX => self.unbox_value(instruction),
    ...
    fn const_i64(&mut self, instruction: Instruction) {
        let dest = InstructionDecoder::decode_dest(instruction);
        let index = InstructionDecoder::decode_const19(instruction);

        let constant = &self.constants[index as usize];

        let Constant::I64(value) = constant else {
            panic!("expected a constant i64 but found {:?}", constant)
        };

        self.set_i64_in_register(dest, *value);
    }

    fn const_f64(&mut self, instruction: Instruction) {
        let dest = InstructionDecoder::decode_dest(instruction);
        let index = InstructionDecoder::decode_const19(instruction);

        let constant = &self.constants[index as usize];

        let Constant::F64(value) = constant else {
            panic!("expected a constant f64 but found {:?}", constant)
        };

        self.set_f64_in_register(dest, *value);
    }
    ...
    #[inline]
    fn set_f64_in_register(&mut self, index: Instruction, value: f64) {
        self.set_register(index, value.to_bits());
    }

    #[inline]
    fn get_f64_in_register(&mut self, index: Instruction) -> f64 {
        f64::from_bits(self.get_register(index))
    }
